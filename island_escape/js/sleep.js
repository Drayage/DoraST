...
function checkSleepDanger(){
  ...
  if(G.islandId==='caldera'&&G.calderaFinale){
    ...
    const finalePen=Math.max(0,(hunDef>0?15+hunDef:0)+(thiDef>0?20+thiDef:0)+5*poisonN-debrisCnt);
...
function doSleep(){
  ...
  if(G.islandId==='caldera'&&G.calderaFinale&&!G.over){
    ...
    // ── 잔해: 덱+버림덱의 잔해 수만큼 HP 피해 차단 ──
    const debrisCnt=allCards().filter(c=>c.id==='debris').length;
    if(debrisCnt>0&&finalePen>0){
      const blocked=Math.min(finalePen,debrisCnt);
      finalePen=Math.max(0,finalePen-blocked);
      log(`🪨 잔해 ${debrisCnt}장으로 열기를 막았다. HP 피해 -${blocked} 차단`,'success');
    }
